...
class PokerServiceTest {
    ...
    @Test
    void sendWebsocketMessageSpecialToOnePlayer() throws Exception {
        //given
        WebSocketSession webSocketSessionMock1 = mock(WebSocketSession.class);
        WebSocketSession webSocketSessionMock2 = mock(WebSocketSession.class);
        pokerService.tableMap.put(GAME_KEY_1, TABLE_WITH_TABLEMASTER_AND_TWO_PLAYERS);
        pokerService.getTableById(GAME_KEY_1).getWebsocketsession().put(1L, webSocketSessionMock1);
        pokerService.getTableById(GAME_KEY_1).getWebsocketsession().put(2L, webSocketSessionMock2);

        //when
        pokerService.sendWebsocketMessageSpecial(WEBSOCKET_MESSAGE, 1L, GAME_KEY_1, true);

        verify(webSocketSessionMock1, times(1)).sendMessage(new TextMessage(WEBSOCKET_MESSAGE));
        verify(webSocketSessionMock2, times(0)).sendMessage(new TextMessage(WEBSOCKET_MESSAGE));
    }

    @Test
    void sendWebsocketMessageSpecialToEveryoneBesidesTablemaster() throws Exception {
        // given
        WebSocketSession webSocketSessionMock1 = mock(WebSocketSession.class);
        WebSocketSession webSocketSessionMock2 = mock(WebSocketSession.class);
        WebSocketSession webSocketSessionMock3 = mock(WebSocketSession.class);
        pokerService.tableMap.put(GAME_KEY_1, TABLE_WITH_TABLEMASTER_AND_TWO_PLAYERS);
        pokerService.getTableById(GAME_KEY_1).getWebsocketsession().put(1L, webSocketSessionMock1);
        pokerService.getTableById(GAME_KEY_1).getWebsocketsession().put(2L, webSocketSessionMock2);
        pokerService.getTableById(GAME_KEY_1).getWebsocketsession().put(3L, webSocketSessionMock3);

        // when
        pokerService.sendWebsocketMessageSpecial(WEBSOCKET_MESSAGE, 2L, GAME_KEY_1, false);
        // then
        verify(webSocketSessionMock1, times(1)).sendMessage(new TextMessage(WEBSOCKET_MESSAGE));
        verify(webSocketSessionMock2, times(0)).sendMessage(new TextMessage(WEBSOCKET_MESSAGE));
        verify(webSocketSessionMock3, times(1)).sendMessage(new TextMessage(WEBSOCKET_MESSAGE));
    }
}
